...
require_once realpath( ABSPATH . "app/models/admin/t/TModel.php");
...
public  function getOwnBotList(){ 
    ...
    if ($valCSRF=='okToken') {
        $response=$this->tModel->getOwnBotList($userID); 
    } else {
...
public  function addBot(){ /// falta el user permision
  ...
  $valCSRF=val_CSRF();
  if ($valCSRF=='okToken') {
            $response=$this->tModel->addBot($_SESSION['userID']); 
  } else {
            $response = array('success' => false, 'message' => $valCSRF);
  }
  ...
  $userPermissions=$this->userController->userPermissions($_POST['botID']);
    ...
    if ($valCSRF=='okToken') {
        if ($userPermissions==true) {
             $response=$this->tModel->updateBot();
        } else { 
...
public  function getBotCategoryList() {
    $response=$this->tModel->getBotCategoryList(); 
    return $response;
}

public  function getBotTvsC() {
    $response=$this->tModel->getBotTvsC(); 
    return $response;
}
...
public  function getBotDetails($params) {
    ...
    if ($valCSRF=='okToken') {
        if ($userPermissions==true) {
             $response=$this->tModel->getBotDetails($params['itemID']);
        } else { 
            $response = array('success' => false, 'message' => 'notPermission');
        }
    } else {
...
public  function getFlowTvsC($params) {
    $response=$this->tModel->getFlowTvsC(($params['itemID'])); 
    return $response;
}
...
public  function getBotFLowList($params){ 
    ...
    if ($valCSRF=='okToken') {
        $response=$this->tModel->getBotFLowList($botID); 
    } else {
        $response = array('success' => false, 'message' => $valCSRF);
    }
...
public  function updateWtsCnt(){
  ...
  $userPermissions=$this->userController->userPermissions($_POST['botID']);
    ...
    if ($valCSRF=='okToken') {
        if ($userPermissions==true) {
             $response=$this->tModel->updateWtsCnt();
        } else { 
            $response = array('success' => false, 'message' => 'notPermission');
        }
    } else {
