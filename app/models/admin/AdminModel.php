...
// Manejo de solicitudes de registro
  public function getTotalUser() {
   try {
    $pdo = getPDOInstance();
    $stmt = $pdo->prepare(" SELECT status, COUNT(*) AS total_by_status FROM users GROUP BY status;" );
    $stmt->execute();
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $total_users=$total_verify=$total_unverify=$total_suspended=$active_users_count=0;
    $status_counts['suspended']=0;
    if ($rows) {
        foreach ($rows as $user) {
        $total_users += $user['total_by_status'];
        $total_verify += $user['total_by_status'];
        $status_counts[$user['status']] = $user['total_by_status'];
          //unverify
          //verify
          //suspended
    }
    $stmt = $pdo->prepare(" SELECT COUNT(DISTINCT owner_id) AS active_users_count FROM bots WHERE is_active = 1;" );
    $stmt->execute();
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    if ($rows) {
      $active_users=$rows[0]['active_users_count'];
    }
      
        $response = array('status' => 'success', 'total_users' => $total_users, 'active_users'=>$active_users, 'verify'=>$status_counts['verify'],'unverify'=>$status_counts['unverify'], 'suspended'=>$status_counts['suspended']);
        return $response;
      } else {
        $response = array('success' => false, 'message' => 'noUsers');
      }
    }
...
public function getTotalBots() {
  ...
  // veo cuantas tiendas tiene
    ...
    //creo la membrecía del negocio
     $stmt = $pdo->prepare("INSERT INTO memberships (bot_id) VALUES (:bot_id)");
     $stmt->bindParam(':bot_id', $bot_id);
     $stmt->execute();
     ...
     $message_sequence=json_encode([
      [
        "id" => 1,
        "type" => "text",
        "content" => "¡Hola! Bienvenido a *BeBot*.\n¿En qué puedo ayudarte hoy?"
    ]
    ]) ;
